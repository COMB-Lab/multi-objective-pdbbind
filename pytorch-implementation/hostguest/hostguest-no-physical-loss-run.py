...
class PhysicsInformedLoss(nn.Module):
    ...
    # Changing version to have an elif for the number of physics features
    def forward(self, predictions, targets, model_vars, physics_info):
        ...
        # Calculate physics difference and signs
        physics_diff = (host_energy - guest_energy).squeeze(-1)
        physics_sign = torch.sign(physics_diff)
        model_sign = torch.sign(model_vars.squeeze(-1))
        
        # Sign mismatch penalty
        sign_product = physics_sign * model_sign
        sign_penalty = torch.mean(torch.relu(-sign_product))
        
        # Physics consistency loss (optional - can add later)
...
def featurize(molecule, info):
    ...
    for atom in molecule.GetAtoms():
        ...
        # Trying to make this exactly like the tensorflow version by adding mass, and changing indices a bit
        #new_feature += [atom.GetMass()]
        
        # Add position information
        position = molecule.GetConformer().GetAtomPosition(atom.GetIdx())
        new_feature += [atom.GetMass(), atom.GetAtomicNum(), atom.GetFormalCharge()]
        # This is changing to be indices 33-35
        new_feature += [position.x, position.y, position.z]
        # Trying to get this to be indices 36-37
        #new_feature += [atom.GetAtomicNum(), atom.GetFormalCharge()]
        ...
        # Try changing to -1.0 padding to indicate no neighbor since it seems to be creating false edges
...
# changing lr to be smaller (0.00001) to see if that helps with training stability
def train_model(model, X_train, y_train, X_val, y_val, epochs=100, lr=0.005, device='cpu',
...
def main():
    ...
    print("-" * 80)
    # Changing c_out_channel to 128 to test rmse improvement
    model = PGGCNModel(num_atom_features=36, r_out_channel=20, c_out_channel=128)

    # Add rules (changing to match tensorflow version)
    model.add_rule("sum", 0, 32) # changes from 0, 30
    model.add_rule("multiply", 32, 33) # changes from 30, 31
    model.add_rule("distance", 33, 36) # Already correct
