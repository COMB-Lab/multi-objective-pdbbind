"""
PyTorch PGGCN Training Script - Improved Version with Enhanced Reporting
...
def train_model(model, X_train, y_train, X_val, y_val, 
                use_pcgrad=False, use_physics=True, config=None, device='cpu'):
    """
    Train the PGGCN model
    
    Args:
        use_pcgrad: If True, use PCGrad optimizer. If False, use standard additive loss.
        use_physics: If True, include physics consistency loss. If False, only empirical loss.
    """
    if config is None:
        config = Config()
    ...
    for epoch in range(config.EPOCHS):
        ...
        train_empirical, train_weighted_physics, train_raw_physics = compute_task_losses(
            predictions, y_train_tensor, physics_info, config.PHYSICS_WEIGHT if use_physics else 0.0
        )
        ...
        if use_physics:
            ...
            # No physics loss - only empirical
            optimizer.zero_grad()
            train_empirical.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
            optimizer.step()
        ...
        if use_physics:
            ...
            train_loss_total = train_empirical.item()
        ...
        with torch.no_grad():
            ...
            val_empirical, val_weighted_physics, val_raw_physics = compute_task_losses(
                val_predictions, y_val_tensor, val_physics_info, config.PHYSICS_WEIGHT if use_physics else 0.0
            )
            if use_physics:
                val_loss_total = val_empirical.item() + val_weighted_physics.item()
            else:
                val_loss_total = val_empirical.item()
        ...
        # Print progress - Enhanced format with all losses
        if (epoch + 1) % 10 == 0 or epoch == 0:
            elapsed = time.time() - start_time
            eta = (elapsed / (epoch + 1)) * (config.EPOCHS - epoch - 1)
            
            print(f"Epoch {epoch+1:3d}/{config.EPOCHS}")
            print(f"  Train -> Total: {train_loss_total:.4f}, RMSE: {train_empirical.item():.4f}, Phys: {train_raw_physics.item():.4f}")
            print(f"  Val   -> Total: {val_loss_total:.4f}, RMSE: {val_empirical.item():.4f}, Phys: {val_raw_physics.item():.4f}")
...
def print_comparison_table(results):
    """
    Print a LaTeX-style comparison table similar to the paper
    
    Args:
        results: Dictionary with keys like 'PCGrad', 'Adam', 'No Physics'
    ...
    # Print ASCII table
    print(f"\n{'Model Type':<30} | {'Mean Train Loss':<20} | {'Mean Test Loss':<20} | {'MAE':<10}")
    print("-" * 90)
    ...
    print(r"  \centering")
    print(r"  \small")
    print(r"  \begin{tabular}{p{4.0cm} p{2.0cm} p{2.0cm} p{1.5cm}}")
    print(r"    \toprule")
    print(r"    \textbf{Model Type} & \textbf{Mean Train Loss} & \textbf{Mean Test Loss} & \textbf{MAE} \\")
    print(r"    \midrule")
...
def main():
    ...
    parser.add_argument('--run-all', action='store_true',
                       help='Run all three configurations: PCGrad, Adam, and No Physics')
    ...
    if args.run_all:
        ...
        runs = [('PCGrad' if args.use_pcgrad else 'Adam', args.use_pcgrad, True)]
    ...
    for run_name, use_pcgrad, use_physics in runs:
        ...
        history = train_model(model, X_train, y_train, X_test, y_test,
                            use_pcgrad=use_pcgrad, use_physics=use_physics, 
        ...
        filename = f"pggcn_{suffix}_{config.PHYSICS_WEIGHT}.pth"
