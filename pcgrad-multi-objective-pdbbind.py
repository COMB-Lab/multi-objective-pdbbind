...
class PGGCN_Hybrid(tf.keras.Model):
    ...
    def call(self, inputs, training=True):
        ...
        for i in range(batch_size):
            # Get single sample - shape: [max_atoms, 53]
            sample = inputs[i]
            
            # Extract only atom features (first 38 dimensions)
            atom_features = sample[:, :38]  # Shape: [max_atoms, 38]
            
            # Determine actual number of atoms
            ...
            # Process through graph convolution layer
            x_list = self.ruleGraphConvLayer([atom_features_actual])
            
            # Process through conv layer
            x = self.conv(x_list)
            
            conv_outputs.append(x)
        
        # Stack all batch outputs
        x = tf.concat(conv_outputs, axis=0)  # Shape: [batch_size, c_out_channel]
        ...
        model_var = self.dense6(x)  # Shape: [batch_size, 1]
        
        # Merge model prediction with physics information
        merged = tf.concat([model_var, physics_info], axis=1)  # Shape: [batch_size, 16]
        out = self.dense7(merged)  # Shape: [batch_size, 1]
