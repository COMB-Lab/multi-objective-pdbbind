"""
Grid Search for Physics Weight with PCGrad
Efficient GPU implementation - searches 0.0 to 1.0 in 0.01 increments (101 values)
...
def train_with_weight(model, X_train, y_train, X_test, y_test, 
    ...
    for epoch in range(config.EPOCHS):
        ...
        # Print progress every 50 epochs so user knows it's working
        if epoch == 0 or (epoch + 1) % 50 == 0:
            print(f"      Epoch {epoch+1}/{config.EPOCHS}: Loss {train_empirical.item():.4f}", flush=True)
...
def main():
    print("="*80)
    print("GRID SEARCH: PHYSICS WEIGHT (0.0 to 1.0, step 0.01)")
    ...
    if device == 'cpu':
        ...
        print(f"  Expected time: ~10-15 hours for all 101 weights\n")
